...
int main()
{
    ...
    compiler_options.push_back("-I" + baseDir);
    compiler_options.push_back("-I" + baseDir + "libs/hiprt");
    compiler_options.push_back(
        "-DNO_VECTOR_OP_OVERLOAD");  // A workaround for operator overload
                                     // conflicts with HIPRT library.

    if (isNvidia) { compiler_options.push_back(NV_ARG_LINE_INFO); }
    else { compiler_options.push_back(AMD_ARG_LINE_INFO); }
#if defined(DEBUG_GPU)
    if (isNvidia) { compiler_options.push_back("-G"); }
    else { compiler_options.push_back("-O0"); }
#endif
    ...
    const std::filesystem::path shader_path = {baseDir +
                                               "/examples/07_pt/07_pt.cu"};
    Shader shader(shader_path.generic_string().c_str(), "07_pt.cu",
                  compiler_options, UseHiprt(hContext).func("path_trace"));
    ...
    std::vector<Triangle> triangles =
        loadTrianglesFromObj("../assets/blocks_pt.obj", "../assets/");

    // カメラの初期化
    // blocks_pt.obj
    camera.m_cameraOrig = float3{5.983407, 13.970583, -28.553869};
    camera.m_cameraLookat = float3{-5.354514, 4.815835, -2.047728};
