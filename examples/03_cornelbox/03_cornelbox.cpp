...
int main()
{
    ...
    const std::filesystem::path shader_path = {
        baseDir + "/examples/03_cornelbox/03_cornelbox.cu"};
    Shader shader(shader_path.generic_string().c_str(), "03_cornelbox.cu",
                  options);
    ...
    std::vector<Triangle> triangles =
        loadTrianglesFromObj("../assets/cornellbox1.obj", "../assets/");
    ...
    while (glfwWindowShouldClose(window) == 0)
    {
        ...
        float3 v0 = {0.2, 2.0f, 0.3f};
        float3 v1 = {3.2, 4.0f, 0.1f};
        float3 v2 = {0.2f, 0.2f, -0.3f};
